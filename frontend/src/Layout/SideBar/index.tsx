  ...
  return (
    <>
      <div className="bg-emerald-950 h-screen  relative">
        <div className="ps-5 py-3 hidden md:flex">
          <Logo isSideBar={true} />
        </div>
          ...
          {Links.map((link) => (
            <NavLink to={link.path} key={link.name}>
              {({ isActive }) => (
                <li
                  className={` ${
                    isActive ? "bg-orange-500 text-white" : "text-gray-200 "
                  } flex gap-3 text-bold text-md px-5 py-3 `}
                >
                  {link.icon}
        ...
        <NavLink to="/instructor/signOut">
          {({ isActive }) => (
            <div
              className={` ${
                isActive ? "bg-orange-500 text-white" : "text-gray-200 "
              } flex gap-3 text-bold text-md px-5 py-3 absolute bottom-0 w-full`}
            >
              <LogOut size={20} />
