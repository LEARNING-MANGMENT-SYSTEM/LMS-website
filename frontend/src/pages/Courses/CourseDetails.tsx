    ...
    <div className="col-span-8">
      ...
      <Section
        ...
        {courseData.feedback.length === 0 ? (
          <p className="text-orange-300 text-center text-xl p-5 ">No Feedback provided</p>
        ) : (
          <>
            {courseData.feedback.map((feedback) =><div className="h-50"> feedback component</div>)}
            <Button variant="secondary">Load More</Button>
          </>
        )}
      </Section>
      <Section title="Related Course" actions={<Button variant="secondary" className="items-center">View All <ArrowRight color="orange" /></Button>}>
        <div className="flex gap-2 justify-center mb-10">
          {courseData.realatedCourse.map(course => <CourseCard {...course}/>)}
        </div>
      </Section>
    </div>
