...
import { courseCategories, courseDurations, courseLanguages, courseSubCategories, subtitleLanguages } from "./data";
...
const BasicForm = ({initialValues=defaultValues}:props) => {
  ...
  const [courseSubCategory,setSubCourseCategory]=useState(initialValues.courseSubCategory)
  const [courseLanguage,setCourseLanguage]=useState(initialValues.courseLanguage)
  const [subtitleLanguage, setSubtitltLanguage]=useState(initialValues.subtitleLanguage)
  ...
  const handleSubmit=(e:React.FormEvent<EventTarget>)=>{
    e.preventDefault();
      console.log(title,subtitle,courseTopic,courseCategory,courseLevel,
      courseSubCategory,duration,subtitleLanguage,courseLanguage);
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        <Input label="Title" placeholder="your course title" onChange={(e)=>setTitle(e.target.value)} value={title} />
        <Input label="Subtitle" placeholder="your course subtitle" onChange={(e)=>setSubTitle(e.target.value)} value={subtitle}/>
        ...
        <div className="grid md:grid-cols-4 grid-cols-1 gap-6">
          ...
          <Select
            ...
            handleChange={(duration)=>setDuration(duration)}
            selectedValue={duration}
          />
        </div>

        <div className="flex justify-between p-5">
          <Button variant={"text"} onclick={handleDelete} >Cancel</Button>
